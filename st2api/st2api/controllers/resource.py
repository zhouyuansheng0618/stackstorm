...
from st2common.models.db.rbac import PermissionGrantDB, RoleDB
from st2common.models.db.pack import PackDB
from st2common.models.db.action import ActionDB
from collections import Counter

from st2common.util.author_role import judge
...
# 获取用户的的权限list
def get_role_list_id(role):
    role_list = RoleDB.objects(name=role).first()
    permission_grants_id_list = role_list['permission_grants']
    return permission_grants_id_list


# 获取权限的pack_id
def get_pack_id(resource_type):
    result_id_list = []
    result_dict = {}
    permission_grant_list = PermissionGrantDB.objects(resource_type=resource_type).all()
    for permission_grant in permission_grant_list:
        if resource_type+'_view' or resource_type+"_all" in permission_grant['permission_types']:
            id = permission_grant['id'].__str__()
            result_id_list.append(id)
            result_dict[id] = permission_grant['resource_uid']

    return result_id_list, result_dict

# 获取pack 创建的权限
def get_create_pack_id(resource_type):
    result_id_list = []
    result_dict = {}
    permission_grant_list = PermissionGrantDB.objects(resource_type=resource_type).all()
    for permission_grant in permission_grant_list:
        if resource_type+'_create' in permission_grant['permission_types'] or resource_type+"_all" in permission_grant['permission_types']:
            id = permission_grant['id'].__str__()
            result_id_list.append(id)
            result_dict[id] = permission_grant['resource_uid']
    ...
    return result_id_list, result_dict
# 获取pack权限的id
def get_pack_permission_id(resource_type,author):
    permission_id_list, result_dict = get_pack_id(resource_type=resource_type)
    role_id_list = get_role_list_id(role=author)
    id_list = list(set(permission_id_list) & set(role_id_list))
    result = []
    for id in id_list:
        pack_obj = PackDB.objects(uid=result_dict[id]).first()
        if pack_obj:
            result.append(pack_obj['id'].__str__())        
    return result
# 获取pack create 权限的id
def get_pack_create_permission_id(resource_type,author):
    permission_id_list, result_dict = get_create_pack_id(resource_type=resource_type)
    role_id_list = get_role_list_id(role=author)
    id_list = list(set(permission_id_list) & set(role_id_list))
    result = []
    for id in id_list:
...
# 获取pack权限的id
def get_action_permission_id(resource_type,author):
    permission_id_list, result_dict = get_pack_id(resource_type=resource_type)
    role_id_list = get_role_list_id(role=author)
    id_list = list(set(permission_id_list) & set(role_id_list))
    result = []
...
class ResourceController(object):
    ...
    mandatory_include_fields_response = ["id", "uid"]
    ...
    def _get_pack_all(
        ...
        if dict(Counter(['name', 'description', 'id']))==dict(Counter(include_fields)):
            pack_uid = get_pack_create_permission_id(resource_type='pack',author=author)
        else:
            pack_uid = get_pack_permission_id(resource_type='pack',author=author)
        result_list = []
        for i in result:
            if i.id in pack_uid:
                result_list.append(i)
        resp = Response(json=result_list)
#        resp.headers["X-Total-Count"] = str(instances.count())
        resp.headers["X-Total-Count"] = str(len(result_list))

        if limit:
            resp.headers["X-Limit"] = str(limit)

        return resp
    def _get_pack_role_all(
                self,
                exclude_fields=None,
                include_fields=None,
                author=None,
    ...
    def _get_action_all(
        ...
        LOG.info("111111auhtor",author)
        pack_uid = get_action_permission_id(resource_type='pack',author=author)
        result_list = []
        for i in result:
            
            if "pack:"+i.pack in pack_uid:
                result_list.append(i)
...
class ContentPackResourceController(ResourceController):
    ...
    def _get_action_all(
        ...
        if judge(author):
            ...
            resp = super(ContentPackResourceController, self)._get_action_all(
                exclude_fields=exclude_fields,
                include_fields=include_fields,
                author=author,
                sort=sort,
